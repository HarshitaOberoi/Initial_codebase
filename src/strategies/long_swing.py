...
class LongSwingStrategy:
    def __init__(self, config):
        self.config = config
        self.rsi_period = config.get('strategies', {}).get('long_swing', {}).get('rsi_period', 14)
        self.macd_signal_period = config.get('strategies', {}).get('long_swing', {}).get('macd_signal_period', 9)
        self.atr_period = config.get('strategies', {}).get('long_swing', {}).get('atr_period', 14)
        self.min_rsi = config.get('strategies', {}).get('long_swing', {}).get('min_rsi', 40)
        self.rsi_overbought = config.get('strategies', {}).get('long_swing', {}).get('rsi_overbought', 60)
        self.leverage = config.get('strategies', {}).get('long_swing', {}).get('leverage', 1)
        self.risk_reward_ratio = config.get('trading', {}).get('risk_reward_ratio', {}).get('long_swing', 3.0)
        ...
        self.max_spread = config.get('strategies', {}).get('long_swing', {}).get('max_spread', 0.001)  # Max bid-ask spread (0.1%)
        logger.info("LongSwingStrategy initialized with relaxed thresholds")
    ...
    def evaluate(self, data, symbol, timeframe, balance, trade, open_trades=None, order_book=None):
        try:
            if data is None or data.empty or len(data) < max(self.rsi_period, self.atr_period):
                ...
                return 'hold', {}
            latest = data.iloc[-1]
            required_indicators = ['rsi', 'macd', 'macd_signal', 'atr', 'adx', 'volume', 'volume_sma']
            ...
            logger.debug(f"LongSwing for {symbol} ({timeframe}): RSI={latest['rsi']:.2f}, MACD={latest['macd']:.2f}, ADX={latest['adx']:.2f}")
            ...
            # Buy signal
            if not has_open_buy and not has_open_sell and latest['rsi'] <= self.min_rsi and latest['macd'] > latest['macd_signal']:
            ...
            # Sell signal
            if not has_open_buy and not has_open_sell and latest['rsi'] >= self.rsi_overbought and latest['macd'] < latest['macd_signal']:
                stop_loss = entry_price + stop_loss_distance
                take_profit = entry_price - stop_loss_distance * self.risk_reward_ratio
                params = {
                    'entry_price': float(entry_price),
            ...
            # Close buy position
            if has_open_buy and (latest['rsi'] >= self.rsi_overbought or latest['macd'] < latest['macd_signal']):
                stop_loss = entry_price + stop_loss_distance
                take_profit = entry_price - stop_loss_distance * self.risk_reward_ratio
                params = {
                    'entry_price': float(entry_price),
            ...
            # Close sell position
            if has_open_sell and (latest['rsi'] <= self.min_rsi or latest['macd'] > latest['macd_signal']):
                stop_loss = entry_price - stop_loss_distance
                take_profit = entry_price + stop_loss_distance * self.risk_reward_ratio
                params = {
                    'entry_price': float(entry_price),
            ...
            logger.error(f"Error in LongSwingStrategy for {symbol} ({timeframe}): {e}")
            return 'hold', {}