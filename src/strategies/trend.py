...
class TrendStrategy:
    def __init__(self, config):
        self.config = config
        self.rsi_period = config.get('strategies', {}).get('trend', {}).get('rsi_period', 14)
        self.atr_period = config.get('strategies', {}).get('trend', {}).get('atr_period', 14)
        self.min_rsi = config.get('strategies', {}).get('trend', {}).get('min_rsi', 40)
        self.rsi_overbought = config.get('strategies', {}).get('trend', {}).get('rsi_overbought', 60)
        self.leverage = config.get('strategies', {}).get('trend', {}).get('leverage', 2)
        self.risk_reward_ratio = config.get('trading', {}).get('risk_reward_ratio', {}).get('trend', 2.5)
        ...
        self.max_spread = config.get('strategies', {}).get('trend', {}).get('max_spread', 0.001)  # Max bid-ask spread (0.1%)
        logger.info("TrendStrategy initialized with relaxed thresholds")

    def evaluate(self, data, symbol, timeframe, balance, trade, open_trades=None, order_book=None):
        try:
            if data is None or data.empty or len(data) < max(self.rsi_period, self.atr_period):
                logger.warning(f"Insufficient data for {symbol} ({timeframe})")
                return 'hold', {}
            latest = data.iloc[-1]
            required_indicators = ['rsi', 'ema_fast', 'ema_slow', 'macd', 'macd_signal', 'atr', 'adx', 'volume', 'volume_sma']
            ...
            logger.debug(f"Trend for {symbol} ({timeframe}): RSI={latest['rsi']:.2f}, EMA_Fast={latest['ema_fast']:.2f}, MACD={latest['macd']:.2f}, ADX={latest['adx']:.2f}")
            logger.debug(f"Last 10 RSI: {data['rsi'].tail(10).to_list()}, MACD: {data['macd'].tail(10).to_list()}")
            ...
            risk_per_trade = self.config.get('trading', {}).get('risk_per_trade', {}).get(timeframe, 0.035)
            atr_factor = 2.0
            ...
            # Buy signal
            if not has_open_buy and not has_open_sell and latest['rsi'] <= self.min_rsi and latest['ema_fast'] > latest['ema_slow'] and latest['macd'] > latest['macd_signal']:
            ...
            # Sell signal
            if not has_open_buy and not has_open_sell and latest['rsi'] >= self.rsi_overbought and latest['ema_fast'] < latest['ema_slow'] and latest['macd'] < latest['macd_signal']:
                stop_loss = entry_price + stop_loss_distance
                take_profit = entry_price - stop_loss_distance * self.risk_reward_ratio
                params = {
                    'entry_price': float(entry_price),
            ...
            # Close buy position
            if has_open_buy and (latest['rsi'] >= self.rsi_overbought or latest['ema_fast'] < latest['ema_slow'] or latest['macd'] < latest['macd_signal']):
                stop_loss = entry_price + stop_loss_distance
                take_profit = entry_price - stop_loss_distance * self.risk_reward_ratio
                params = {
                    'entry_price': float(entry_price),
            ...
            # Close sell position
            if has_open_sell and (latest['rsi'] <= self.min_rsi or latest['ema_fast'] > latest['ema_slow'] or latest['macd'] > latest['macd_signal']):
                stop_loss = entry_price - stop_loss_distance
                take_profit = entry_price + stop_loss_distance * self.risk_reward_ratio
                params = {
                    'entry_price': float(entry_price),
            ...
            logger.error(f"Error in TrendStrategy for {symbol} ({timeframe}): {e}")
            return 'hold', {}